...
// Vue.use(VueSweetalert2);
 ...
 import { userorder } from "../common/Service";
  ...
  methods: {
    ...
    applyCoupon() {
      if (this.couponCode != undefined) {
        ...
        for (let i in arr) {   ///
          alert("inn");
          console.log(arr[i].couponcode);
          if (arr[i].couponcode == this.couponCode) {
            alert('same');
            console.log(arr[i]);
    ...
    postOrders() {
      this.submitted = true;
        ...
        let formData = {
          firstname: this.user.userfName,
          lastname: this.user.userlName,
